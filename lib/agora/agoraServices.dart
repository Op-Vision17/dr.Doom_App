...
class AgoraService {
  ...
  static Future<String?> startMeeting(
    ...
    try {
      ...
      return tokendata['token']; // Return the fetched token
    } catch (e) {
  ...
  static Future<void> joinMeeting(
      String roomName, uid, String username, String token) async {
    if (_engine == null) {
      throw Exception("Engine not initialized");
    }
    ...
    try {
      await createMember(username, uid, roomName);
      // Join channel with the provided token
      await _engine!.joinChannel(
        token: token,
        channelId: roomName,
        uid: uid,
  ...
  static Future<void> leaveChannel(
    ...
    try {
      await leaveMeeting(username, uid, roomName);
  ...
  static Widget localVideo(int uid) {
    if (_engine != null) {
